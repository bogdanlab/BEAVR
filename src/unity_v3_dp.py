...
def smart_start(z_list, N):
    ...
    for z_b in z_list:
        ...
        print zscores_b
        causal_inds_list_b = np.where(np.absolute(zscores_b) >= 2.0)
...
def main():
    ...
    parser.add_option("--H_snp", "--H_snp", dest="H_snp")
    parser.add_option("--H_gwas", "--H_gwas", dest="H_gwas")
    ...
    if DP == 'n':
        ...
        p_est, p_var, p_list = gibbs_ivar_gw_dp(z_list, H_snp, H_gw, N, ld_half_flist, p_init=p_init, c_init_list=c_init_list, gamma_init_list=gamma_init_list, its=ITS)
